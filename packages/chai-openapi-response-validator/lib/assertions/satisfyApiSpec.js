...
module.exports = function (chai, openApiSpec) {
  ...
  Assertion.addProperty('satisfyApiSpec', function () {
    ...
    this.assert(
      predicate,
      getExpectedResToSatisfyApiSpecMsg(actualResponse, openApiSpec, validationError),
      getExpectedResNotToSatisfyApiSpecMsg(actualResponse, openApiSpec, validationError),
    );
  });
};
...
function getExpectedResToSatisfyApiSpecMsg(actualResponse, openApiSpec, validationError) {
  ...
  if (['PATH_NOT_FOUND', 'SERVER_NOT_FOUND'].includes(validationError.code)) {
    ...
    let msg = 'expected res to satisfy API spec'
  ...
  if (validationError.code === 'METHOD_NOT_FOUND') {
    ...
    return c`expected res to satisfy API spec
        ...
        \n\nRequest operations found for path '${path}' in API spec: ${Object.keys(expectedPathItem).map((op) => op.toUpperCase()).join(', ')}`;
  }
  ...
  return c`expected res to satisfy API spec
      ...
      \n\nThe '${status}' response defined for endpoint '${endpoint}' in API spec: ${stringify(responseDefinition)}`;
}

...
const PathParamFinder = require('path-parser').default;
  ...
  return function (chai) {
    ...
    Assertion.addProperty('satisfyApiSpec', function () {
      ...
      this.assert(
        ...
        `expected res not to satisfy API spec for '${expectedResStatus}' response defined for endpoint '${res.req.method} ${getPathname(res.req, openApiSpec)}' in OpenAPI spec\nres: ${responseSummary}`,
      );
    });
  };
};
...
function checkApiSpecDefinesExpectedResponse(res, expectedResStatus, openApiSpec) {
  const pathname = getPathname(res.req, openApiSpec);

  const routeObj = openApiSpec.paths[pathname];
  if (!routeObj) {
    throw new Error(`No '${pathname}' route defined in OpenAPI spec`);
  }

  const httpMethod = res.req.method;
  const endpointObj = routeObj[httpMethod.toLowerCase()];
  if (!endpointObj) {
    throw new Error(`No '${res.req.method}' method defined for route '${pathname}' in OpenAPI spec`);
  }

  const expectedResponse = endpointObj.responses[expectedResStatus];
  if (!expectedResponse) {
    throw new Error(`No '${expectedResStatus}' response defined for endpoint '${httpMethod} ${pathname}' in OpenAPI spec`);
  }
}
...
function extractExpectedResponsesFromApiSpec(res, openApiSpec) {
  const { req } = res;
  const pathname = getPathname(req, openApiSpec);
  const { method } = req;
  return openApiSpec.paths[pathname][method.toLowerCase()].responses;
}

function getPathname(req, openApiSpec) {
  const pathname = url.parse(req.path).pathname; // excludes the query (because: path = pathname + query)
  const routeObj = openApiSpec.paths[pathname];
  if (routeObj) {
    return pathname;
  }
  const OASPath = parsePathnameAsOASPath(pathname, openApiSpec);
  return OASPath || pathname;
}

function parsePathnameAsOASPath(pathname, openApiSpec) {
  const pathsInApiSpec = Object.keys(openApiSpec.paths);

  const OASPathsWithPathParams = pathsInApiSpec.filter(path => path.includes('{'));

  for (const OASPath of OASPathsWithPathParams) {
    const colonPath = OASPath.replace(/{/g, ':').replace(/}/g, ''); // converts all {foo} to :foo
    const paramFinder = new PathParamFinder(colonPath);
    const pathParamsFoundInPathname = paramFinder.test(pathname);
    if (pathParamsFoundInPathname) {
      return OASPath;
    }
  }
}

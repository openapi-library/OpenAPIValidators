...
function parseOpenApiFile(filePath) {
  ...
  try {
    ...
    const OASpecValidator = new OpenAPISchemaValidator({ version: openApiVersion });
    const { errors } = OASpecValidator.validate(openApiSpec);
...
function findResExpectedByApiSpec(res, OASPathsObj) {
  ...
  expectedResponseObj[status] = expectedResponse;
  return expectedResponseObj;
}
...
function findOASPathMatchingPathname(pathname, OASPaths) {
  ...
  for (const OASPath of OASPathsWithPathParams) {
    ...
    const pathParamFinder = new PathParamFinder(pathInColonForm);
    const pathParamsInPathname = pathParamFinder.test(pathname);

...
class OpenApi3Spec extends AbstractOpenApiSpec {
  ...
  findOpenApiPathMatchingPathname(pathname) {
    ...
    if (!matchingServerBasePaths.length) {
      throw new ValidationError('SERVER_NOT_FOUND', `No server matching '${pathname}' path defined in OpenAPI spec`);
    }
    ...
    if (!openApiPath) {
      ...
      throw new ValidationError('PATH_NOT_FOUND', `No '${pathname}' path defined in OpenAPI spec. (Matches servers ${inspect(matchingServerUrls)} but no 'server/endpointPath' combinations)`);
    }
    return openApiPath;
  }

...
class OpenApi3Spec extends AbstractOpenApiSpec {
  ...
  findOpenApiPathMatchingPathname(pathname) {
    ...
    if (!matchingServerBasePaths.length) {
      throw new ValidationError('MATCHES_NO_SERVERS', `No server matching '${pathname}' path defined in OpenAPI spec`);
    }
    ...
    if (!openApiPath) {
      ...
      throw new ValidationError('MATCHES_NO_PATHS', `No '${pathname}' path defined in OpenAPI spec. (Matches servers ${inspect(matchingServerUrls)} but no 'server/endpointPath' combinations)`);
    }
    return openApiPath;
  }

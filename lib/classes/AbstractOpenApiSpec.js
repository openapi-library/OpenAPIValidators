...
class OpenApiSpec {
  ...
  findExpectedResponse(actualResponse) {
    ...
    if (!expectedResponse) {
      throw new ValidationError('STATUS_NOT_FOUND', `No '${status}' response defined for endpoint '${actualRequest.method} ${this.findOpenApiPathMatchingRequest(actualRequest)}' in OpenAPI spec`);
    }
  ...
  findExpectedResponseOperation(actualRequest) {
    ...
    if (!operationObject) {
      throw new ValidationError('METHOD_NOT_FOUND', `No '${actualRequest.method}' method defined for path '${openApiPath}' in OpenAPI spec`);
    }
  ...
  validateObject(actualObject, schemaName) {
    ...
    if (!schema) {
      return new ValidationError('SCHEMA_NOT_FOUND', `No schema named '${schemaName}' in OpenAPI spec`);
    }
    ...
    let validationError = resValidator.validateResponse(
      mockResStatus,
      actualObject,
    );
    if (validationError) {
      validationError = JSON.parse(JSON.stringify(validationError).replace('response', 'object'));
      return new ValidationError(
        'INVALID_BODY',
        validationError.message,
        validationError,

...
class OpenApiSpec {
  ...
  findExpectedResponse(actualResponse) {
    ...
    if (!expectedResponse) {
      throw new ValidationError('WRONG_STATUS', `No '${status}' response defined for endpoint '${actualRequest.method} ${this.findOpenApiPathMatchingRequest(actualRequest)}' in OpenAPI spec`);
    }
  ...
  findExpectedResponseOperation(actualRequest) {
    ...
    if (!operationObject) {
      throw new ValidationError('WRONG_METHOD', `No '${actualRequest.method}' method defined for path '${openApiPath}' in OpenAPI spec`);
    }

...
function findResExpectedByApiSpec(res, openApiSpec) {
  ...
  if (expectedResponse && expectedResponse['$ref']) {
    const nameOfReferencedResponseDefinition = expectedResponse['$ref'].split('#/components/responses/')[1];
    expectedResponse = openApiSpec.components.responses[nameOfReferencedResponseDefinition];
  }
  if (!expectedResponse) {
